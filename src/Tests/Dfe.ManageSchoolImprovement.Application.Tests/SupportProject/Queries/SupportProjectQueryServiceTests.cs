...
namespace Dfe.ManageSchoolImprovement.Application.Tests.SupportProject.Queries
{
    public class SupportProjectQueryServiceTests
    {
        ...
        [Fact]
        public async Task GetBaseSupportProject_ShouldReturnMappedDto_WhenProjectExists()
        {
            // Arrange
            var project = GetSchoolProjects()[0];
            var supportProjectDto = GetSupportProjectDtos(GetSchoolProjects())[0];
            ...
            _mockRepository.Setup(r =>
                    r.GetSupportProjectBaseById(It.IsAny<SupportProjectId>(), It.IsAny<CancellationToken>()))!
                .ReturnsAsync(project);

            _mockMapper.Setup(m => m.Map<SupportProjectDto?>(project)).Returns(supportProjectDto);

            // Act
            var result = await _service.GetBaseSupportProject(1, CancellationToken.None);

            // Assert
            Assert.True(result.IsSuccess);
            Assert.NotNull(result.Value);
            _mockRepository.Verify(
                r => r.GetSupportProjectBaseById(It.IsAny<SupportProjectId>(), It.IsAny<CancellationToken>()), Times.Once);
        }
        
        [Fact]
        public async Task GetBaseSupportProject_ShouldReturnFailure_WhenProjectNotFound()
        {
            // Arrange
            _mockRepository.Setup(r =>
                    r.GetSupportProjectBaseById(It.IsAny<SupportProjectId>(), It.IsAny<CancellationToken>()))!
