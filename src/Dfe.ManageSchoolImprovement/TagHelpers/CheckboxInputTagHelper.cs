...
public class CheckboxInputTagHelper(IHtmlHelper htmlHelper) : InputTagHelperBase(htmlHelper)
{
   ...
   {
      CheckboxInputViewModel model = new() { Heading = Heading, Id = Id, Name = Name, LabelHint = LabelHint, Label = Label, Value = For.Model?.ToString(), HeadingStyle = HeadingStyle, Email = Email };
